...
{ // compare (readings , string elli feha les X lezm tkoun 2eme parameter)
  if (path[pathSteps] == 'F')
  {
    ...
    Prevc = 'W';
    myledwhiteon();
    mode = 'N'; // FIX #2: assignment (=) not comparison (==)
  }
...
{ // CHANGEMENT DE mode : point of changing found so stop using the mode variable
  ...
  if (path[pathSteps] == 'B') {
    ...
    myledwhiteon();
    currentLineColor = 'B';
    mode = 'N'; // FIX #2: assignment (=) not comparison (==) -- normal mode: NO searching for line-color switching, now normal line following
  } else if (path[pathSteps] == 'W') {

void wallfollow(int offset = 16, char wall = 'L', boolean autoTurn = true) // offset : distance to the wall
{
  int derivative = 0;
  int Tp = 80;
  // FIX #3: gains must be float. As int, values like 3.5, 0.01, 0.1 were
  // truncated to 0, silently killing the wall-following PID terms.
  float Kp2 = 3.5;//6
  float Kd2 = 5;
  // Kp2 = 6; CCV //Kd2 = 30; BBEST
  // Kp2 = 8; CCV //Kd2 = 30;
  // FIX #5: lasterror must persist between calls for the derivative term to
  // mean anything. As a fresh local it was uninitialised every call, making
  // the D term garbage. `static` keeps its value across calls.
  static int lasterror = 0;
  int error = 0, Turn, powerR, powerL;
  int r, l;
  ...
  { // PID FOLLOW
    float localKp2; // FIX #3: float so gains like 0.01 / 12 / 3 aren't truncated
    if (wall == 'R')
    { // right wall follow

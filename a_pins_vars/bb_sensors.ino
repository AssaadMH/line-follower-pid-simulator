...
uint8_t localCountLines(uint8_t cumulativeLineCounter = 0, int index = 0, int prevValue = 0)
{
  ...
  // FIX #1: actually return the recursive result. Without these `return`s the
  // function fell off the end (undefined behaviour) and CountLines() could
  // return garbage on the AVR compiler.
  else if (IntDsensors[index] == 1 && prevValue == 0)
    return localCountLines(cumulativeLineCounter + 1, index + 1, IntDsensors[index]);
  else
    return localCountLines(cumulativeLineCounter, index + 1, IntDsensors[index]);
}
